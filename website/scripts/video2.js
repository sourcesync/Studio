...
function loadVideo(pageFolder, fileNumber, width, height)
{
	...
	if (is_chrome)
	{
		var ret = '<video id="theVideo" width="' + width + '" height="' + height + '" poster="' + 
			...
			pageFolder + '/sd/' + fileNumber + '.mp4video.mp4" />' +
            		'<img src="' + pageFolder + '/' + fileNumber + '.png" width="' + width + '" height="' + height + 
			'" title="No video playback capabilities detected. Use Google Chrome" />' +
        		'</object>' +
   			'</video>';
   		return ret;
	}
